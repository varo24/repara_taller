...
import { 
  ...
  ArrowRight, Activity, Users, ClipboardList, Clock, HardDrive
} from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const pendingDiagnose = repairs.filter(r => r.status === RepairStatus.PENDING);
  const readyToDeliver = repairs.filter(r => r.status === RepairStatus.READY);
  const inProgress = repairs.filter(r => r.status === RepairStatus.DIAGNOSING || r.status === RepairStatus.IN_PROGRESS);
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          ...
          <div className="xl:col-span-4">
            <div className="bg-slate-950 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden border border-white/5">
               <HardDrive size={120} className="absolute -right-8 -bottom-8 opacity-5" />
               <div className="relative z-10">
                 <h3 className="text-sm font-black uppercase tracking-tight mb-4 flex items-center gap-3">
                   <HardDrive size={20} className="text-emerald-400" /> Almacenamiento Local
                 </h3>
                 <p className="text-[11px] text-slate-400 font-bold leading-relaxed uppercase">
                   Tu base de datos reside en este equipo. Los datos de tus clientes nunca salen de este terminal.
                 </p>
                 <div className="mt-8 pt-8 border-t border-slate-800">
                    <div className="flex justify-between items-center mb-2">
                       <span className="text-[9px] font-black text-slate-500 uppercase">Estado del Motor</span>
